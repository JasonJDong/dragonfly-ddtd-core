...
public class PathChildrenCache implements Closeable
{
    ...
    private final AtomicReference<PathChildrenCache.State> state = new AtomicReference<PathChildrenCache.State>(PathChildrenCache.State.LATENT);
    ...
    private volatile Watcher childrenWatcher = new Watcher()
    {
        ...
        {
            offerOperation(new RefreshOperation(PathChildrenCache.this, PathChildrenCache.RefreshMode.STANDARD));
        }
    };
     ...
     * @throws Exception errors
     * @deprecated use {@link #start(PathChildrenCache.StartMode)}
     */
    @Deprecated
    public void start(boolean buildInitial) throws Exception
    {
        start(buildInitial ? PathChildrenCache.StartMode.BUILD_INITIAL_CACHE : PathChildrenCache.StartMode.NORMAL);
    }

    /**
     * Method of priming cache on {@link org.apache.curator.framework.recipes.cache.PathChildrenCache#start(org.apache.curator.framework.recipes.cache.PathChildrenCache.StartMode)}
     */
    public enum StartMode
    {
         ...
         * {@link org.apache.curator.framework.recipes.cache.PathChildrenCache#rebuild()} will be called before
         * the {@link org.apache.curator.framework.recipes.cache.PathChildrenCache#start(org.apache.curator.framework.recipes.cache.PathChildrenCache.StartMode)} method returns
    ...
    {
        Preconditions.checkState(state.compareAndSet(PathChildrenCache.State.LATENT, PathChildrenCache.State.STARTED), "already started");
        ...
        switch ( mode )
        {
            case NORMAL:
            {
                offerOperation(new RefreshOperation(this, PathChildrenCache.RefreshMode.STANDARD));
                break;
            }
            ...
            case POST_INITIALIZED_EVENT:
            {
                ...
                offerOperation(new RefreshOperation(this, PathChildrenCache.RefreshMode.POST_INITIALIZED));
                break;
            }
        }
    }
    ...
    {
        Preconditions.checkState(state.get() == PathChildrenCache.State.STARTED, "cache has been closed");
        ...
        offerOperation(new RefreshOperation(this, PathChildrenCache.RefreshMode.FORCE_GET_DATA_AND_STAT));
    }
    ...
    {
        if ( state.compareAndSet(PathChildrenCache.State.STARTED, PathChildrenCache.State.CLOSED) )
        {
    ...
    /**
     * Clear the current data of assigned path without beginning a new query and without generating any events
     * for listeners.
    ...
    void refresh(final PathChildrenCache.RefreshMode mode) throws Exception
    {
        ensurePath();
            ...
            public void processResult(CuratorFramework client, CuratorEvent event) throws Exception
            {
                ...
                else if ( event.getResultCode() == KeeperException.Code.NONODE.intValue() )
                {
                    ...
                    else
                    {
                        ...
                        ensureContainers.reset();
                        offerOperation(new RefreshOperation(PathChildrenCache.this, PathChildrenCache.RefreshMode.NO_NODE_EXCEPTION));
                    }
                }
            }
        };
    ...
    {
        if ( state.get().equals(PathChildrenCache.State.CLOSED))
        {
            client.removeWatchers();
            return true;
        }
    ...
    private void processChildren(List<String> children, PathChildrenCache.RefreshMode mode) throws Exception
    {
        ...
        for ( String name : children )
        {
            ...
            if ( (mode == PathChildrenCache.RefreshMode.FORCE_GET_DATA_AND_STAT) || !currentData.containsKey(fullPath) )
            {
                            ...
                            new Runnable()
                            {
                                ...
                                public void run()
                                {
                                    ...
                                    catch ( InterruptedException e )
                                    {
                                        ...
                                        if ( state.get() != PathChildrenCache.State.CLOSED )
                                        {
                                            handleException(e);
                                        }
    ...
    {
        if ( state.get() == PathChildrenCache.State.STARTED )
        {
